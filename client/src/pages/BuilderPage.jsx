...
const BuilderPage = () => {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.error || "Publish failed");
    }finally{
      setPublishing(false)
    }
  }
      ...
      <div className='w-[320px] shrink-0 flex flex-col border-r border-zinc-200 bg-white'>
       ...
       <div className='flex border-b border-zinc-100'>
           ...
           <button onClick={()=> setLeftTab("chat")}
            ...
            cursor-medium cursor-pointer ${leftTab === "chat" ? "text-zinc-900 border-b-2 border-zinc-900" :
           ...
           <button onClick={()=> setLeftTab("files")}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium
            cursor-medium cursor-pointer ${leftTab === "files" ? "text-zinc-900 border-b-2 border-zinc-900" :
              "text-zinc-400 hover:text-zinc-700"}`}>
       ...
       <div className='flex-1 min-h-0 overflow-hidden'>
         {
          leftTab === 'chat' ? (
            <div className='h-full min-h-0'><ChatPanel messages={activeProject.messages} onSend={handleChat} Loading={chatLoading}/></div>
          ):(
