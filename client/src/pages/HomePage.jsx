    ...
    bg-cover bg-center bg-no-repeat">
     ...
     <nav className='sticky top-0 z-10 flex items-center justify-between px-6 py-4'>
        ...
        <div className='flex items-center gap-4 text-sm font-medium text-zinc-300'>
          <span>{user?.name}</span>
          <button onClick={logout} className='py-1.5 px-3 border border-white/20 text-white hover:bg-white/10 text-xs rounded-md cursor-pointer 
          bg-transparent'>
            Sign out
          </button>
        </div>
     ...
     <div className='flex-1 flex flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28'>
       <div className='w-full max-w-2xl flex flex-col items-center'>
         ...
         border-white/20 text-[13px] text-white/90'>
         ...
         <p className='text-center text-sm md:text-base maw-w-xl mt-4 text-white/65 leading-relaxed'>
            ...
            <div className='animate-marquee gap-3'>
             {homeTags.map((tag, i)=>(
              <button key={i}
               ...
               className='px-4 py-1.5 border rounded-full text-sm text-white bg-white/10 border-white/25 hover:bg-white/20
               transition cursor-pointer shrink-0 font-medium'>
                {tag}
              </button>
             ))}
            </div>
         ...
         {!loadingProjects && projects.length > 0 && (
          <div className='mt-12 w-full'>
             ...
             <div className='flex items-center justify-between pb-3 mb-3 border-white/10'>
             ...
             <div className='space-y-2 max-h-[80vh] overflow-y-auto pr-1'>
               {projects.map((p)=>(
                <div key={p._id} className='bg-white/5 border border-white-10 rounded-lg px-4 py-3 flex items-center justify-between 
                group hover:bg-white/10 cursor-pointer backdrop-blur-md trasition-all'
